...
Version: 1.5
Abhängigkeit: REQ-022 (Pflegeerinnerungen/OverwinteringProfile/CareProfile — erweitert), REQ-039 (Winterhärte-Ampel + Frost-Defaults), REQ-005 (Wetter-/Frost-Livedaten), REQ-046 (Wetterquellen-Auflösung), REQ-041 (Klimanormale als Saison-Fallback), REQ-002 (Standort — Innen/Außen, GPS, Hemisphäre), REQ-001 (frost_sensitivity), REQ-003 (dormancy-Phase, Invariante D5, §D8 Rollen-Map, §D14 Sequenz-Resolver), REQ-006 (Task-Erzeugung), REQ-013 (Run/Dual-Support), REQ-024 (Mandanten-Scoping)
```
...
|---------|-------|----------|
| 1.5 | 2026-07-19 | **Dormanz-Kopplung der feingetypten Rest-Phasen bestätigt (#611 WS1, Audit #576/#586 → #616):** Klarstellung, wie die von #616 eingeführten Rest-Phasen (`winter_rest`, `summer_rest`, `rest_phase`, `dry_storage`, `winter_hull_change`) an die Überwinterungs-Automatik koppeln (neuer §3.5.1). **(a) Pflege-Kopplung — vollständig:** Der `DormancyCareActivator` schaltet den `dormancy_care_mode` **phasennamen-agnostisch** je Instanz (Pfad A ODER B) beim SeasonState-Übergang → `winter_dormancy`; die feingetypten Rest-Phasen sind über die REQ-003 §D8 Rollen-Map (`phase_role_map.is_rest_phase` → alle fünf → `dormancy`-Rolle) korrekt als Ruhe erkannt (Düngung aus, `dormancy_watering`). **(b) Wachstumsphasen-Kopplung — Lücke dokumentiert:** Der `SeasonPhaseCoupler.enter_dormancy` sucht die Zielphase per **literalem** Namen `dormancy` (`find_phase_key_by_name(species_key, "dormancy")`) und erkennt die abweichend benannten Rest-Phasen daher **nicht** — eine `cam_succulent_rest`-Sukkulente (Terminalphase `winter_rest`) wird vom Standort-Wintersignal nicht aktiv in ihre Ruhe beschleunigt (sie erreicht `winter_rest` nur über die eigene, zeitgetriebene Sequenz-Transition; `restart_cycle` funktioniert dagegen über `cycle_restart_entry_order` korrekt). Remediation-Vorschlag im Master-Data-Capture-Audit (`spec/analysis/master-data-capture-audit-2026-07.md`, Finding NCT-2): `enter_dormancy` gegen die Rollen-Map (`is_rest_phase`/`find_phase_key_by_role`) statt gegen den Literalnamen auflösen. Reine Spec-Klarstellung, keine Code-Änderung in dieser Version. |
...
<!-- Quelle: #611 WS1 (Audit #576/#586 → #616) — Kopplung der feingetypten Rest-Phasen an die Saison-Automatik -->
### 3.5.1 Kopplung der feingetypten Rest-Phasen (D9–D12) an die Überwinterungs-Automatik

Die #616 eingeführten fein-getypten Sequenzen (REQ-003 §D14) verwenden **botanisch benannte Ruhephasen** statt der generischen `dormancy`-Phase: `cam_succulent_rest`/`cam_double_rest` → `winter_rest`/`summer_rest`/`winter_hull_change`, `fern_spore` → `rest_phase`, `geophyte_fine` → `dry_storage`. Für die Saison-Kopplung gilt:

- **Pflege-Kopplung (`DormancyCareActivator`, §3.5) — vollständig.** Der Dormancy-Care-Modus wird beim SeasonState-Übergang → `winter_dormancy` **je Instanz** geschaltet, unabhängig vom Phasennamen. Alle fünf Rest-Phasen sind über die REQ-003 §D8 **Rollen-Map** (`phase_role_map.core_phase`/`is_rest_phase`) auf die `dormancy`-Rolle abgebildet — Düngung ausgesetzt, `dormancy_watering` aus dem `OverwinteringProfile`, `dormancy_health_check`/`quarter_climate_check` aktiv. Die Winterruhe-Pflege greift damit für Sukkulenten, Farne und Geophyten korrekt.

- **Wachstumsphasen-Kopplung (`SeasonPhaseCoupler`, v1.3) — bekannte Lücke.** `enter_dormancy` löst die Zielphase heute per **Literalnamen** `dormancy` auf (`find_phase_key_by_name(species_key, "dormancy")`) und erkennt die abweichend benannten Rest-Phasen daher nicht. Konsequenz: das standortweite Wintersignal beschleunigt eine feingetypte Pflanze **nicht** aktiv in ihre Ruhe — sie erreicht `winter_rest`/`dry_storage` etc. ausschließlich über die **eigene, zeitgetriebene Sequenz-Transition** (`check_auto_transitions` entlang der `phase_sequence`). Der umgekehrte Weg `restart_cycle` (→ `pre_spring`) funktioniert dagegen bereits korrekt, weil er über `resolve_cycle_restart_phase_key` / `cycle_restart_entry_order` (nicht über einen Literalnamen) auflöst.

  **Remediation (empfohlen, nicht in dieser Spec-Schärfung umgesetzt):** `SeasonPhaseCoupler.enter_dormancy` soll die Zielphase über die Rollen-Map bestimmen (erste Phase der gebundenen Sequenz mit `is_rest_phase(name) == True`, Fallback Literal `dormancy`), analog zu `restart_cycle`. Damit koppelt das Standort-Wintersignal auch die feingetypten Rest-Phasen. Verfolgt als Finding **NCT-2** im Master-Data-Capture-Audit (`spec/analysis/master-data-capture-audit-2026-07.md`), Folge-Issue-Vorschlag dort gelistet.
