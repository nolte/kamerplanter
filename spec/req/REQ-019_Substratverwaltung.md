...
- **`:Substrate`** - Substrat-Definition
  - Properties:
    ...
    - `ph_base: float` (pH im **Wasserextrakt**, pH-H₂O)
      ...
      > **Konvention (festgelegt 2026-09-03, #1175):** `ph_base` ist der pH-Wert eines Wasserextrakts, nicht der CaCl₂-Wert. Das Extraktionsverhältnis legt der Katalog nicht fest (Plagron publiziert 1:1,5 v/v nach RHP, EN 13037 misst 1:5 v/v); es wird je Quelle im Seed-Datensatz notiert. Grund: jeder Konsument des Felds stellt ihn einer Messung in Wasser gegenüber — die CEC-gewichtete Mischung aus `calculate_mix_properties`, die MCP-Düngebeurteilung, und die API, die ihn neben der `ph_history` einer Charge (Messgerät im Drain oder in der Aufschlämmung) ausliefert. (`SubstrateLifecycleManager.check_reusability` liest `ph_history` und `ec_base_ms`, aber nicht `ph_base` — gemessen unter #1175.) Ein CaCl₂-Wert läge 0,5–1 Einheit darunter (LECHUZA-PON deklariert 8,2 H₂O gegen 7,1 CaCl₂) und würde als „saurer als dein Messgerät" gelesen. Ein Datensatz, dessen einzige Quelle CaCl₂ angibt, wird **nicht** umgerechnet — der Abstand ist substratabhängig — und bleibt offen.
    - `ec_base_ms: float` (Vorgedüngt oder inert)
    - `water_retention: Literal['low', 'medium', 'high']` — **pro Typ deklariert** (siehe oben), **nicht** aus `water_holding_capacity_percent` abgeleitet
    - `water_holding_capacity_percent: Optional[float]` (volumetrische WHC bei Containerkapazität, pF 1 / EN 13041; quantitativ: low <30%, medium 30–60%, high >60%)

      > **Abgrenzung (festgelegt 2026-09-17, #1175):** Die Bandgrenzen beschreiben die Wertebereiche *dieser Zahl*, sie sind keine Ableitungsregel für `water_retention`. Gemessen am Katalog: liest man sie als Regel, widersprechen 10 der 27 Datensätze mit beiden Feldern — in beide Richtungen (Steinwollmatte 80 % ist `medium`, sechs torfbasierte Erden mit 55–58 % sind `high`). Grund: die Zahl gilt bei einem Matrixpotential und trägt ihre Methode nicht mit sich (getrocknetes Sphagnum liest 28 Vol-% bei pF 1 und 85 Vol-% nach Sättigung und freier Drainage), während das Enum eine Typaussage ist. Seit #1368 ist `water_retention` das einzige Signal des Gießmengen-Modifikators — ein Enum an seine Zahl anzugleichen ändert eine echte Gießempfehlung. Die Typdeklarationen sind in `test_substrate_invariants.py` als Invariante festgehalten.
