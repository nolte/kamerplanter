...
| **REQ-005** | Stellt das `:WeatherForecast`-Basismodell + Frostwarnung bereit; die Historie der Wetter-/Klima-Adapter liefert die Klimanormale. Frost-Richtwerte ergänzen die Frostwarnung als Fallback ohne Live-Wetter. |
| **REQ-046** | SSOT der Wetter-Datenquellen-Schicht: `WeatherAdapter`-ABC + `WeatherAdapterRegistry`, in die sich der `OpenMeteoClimateNormalAdapter` registriert. REQ-039 ist Konsument geteilter Infrastruktur; teilt DWD/Open-Meteo-Quellen + Attributionspflichten. |
