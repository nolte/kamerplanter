...
**Dienstkonten (`account_type: 'service'`, REQ-023 M2M).** Ein Dienstkonto nimmt an diesem Mechanismus **wie ein interaktiver Aufrufer** teil: Es benötigt ein echtes, aktives Membership im benannten Mandanten, und ein Mandant ohne Membership ist von einem nicht existierenden ununterscheidbar (`403`, byte-identisch). Es unterscheidet sich in **genau einem** Punkt — es hat **keinen Rückfall auf einen persönlichen Mandanten**: Ohne Header löst es auf globalen Scope `""` auf. „In welchem Mandanten handelt diese Maschine?" hat keine implizite Antwort, also lautet die Antwort *keiner*.

Diese Ausnahme ist normativ und nicht bloß beschreibend. Heute legt kein Pfad einen persönlichen Mandanten für ein Dienstkonto an, das Verhalten wäre also auch ohne Regel identisch — durch Zufall der Datenlage. Sobald ein Administrator, eine Migration oder ein Fixture einem Dienstkonto einen persönlichen Mandanten gibt, würde jeder header-lose M2M-Aufruf still innerhalb eines Mandanten handeln. Das ist eine Erweiterung ohne Codeänderung, die sie ankündigt — genau das, was der Fail-safe-Grundsatz oben verbietet.
