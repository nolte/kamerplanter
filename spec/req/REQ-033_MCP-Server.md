...
| `validate_nutrient_plan_coverage` | Lesend: welche Phasen der Pflanzenfolge der gebundene Plan **namentlich** deckt, welche nicht, und welche ohne `target_ec_ms` sind | — (Auswertung ueber bestehende Lesepfade) |
...
| `validate_nutrient_plan_coverage` | `list_plants` | — (rein lesend) |
...
- **`validate_nutrient_plan_coverage`** (Issue #1244, Punkt 3) fuegt **keine** Schreibflaeche hinzu
  und macht trotzdem den zweiten Befund dieses Issues erst sichtbar. Die Phasenaufloesung in
  `resolve_effective_entry` sucht zuerst nach **Name plus Wochenfenster**; findet sie nichts, faellt
  sie auf einen reinen **Wochenbereichs-Treffer** zurueck — und der Rueckgabewert verraet nicht,
  welcher Zweig gegriffen hat. Eine Pflanze, die aus der falschen Phase gefuettert wird, sieht damit
  genauso aus wie eine korrekt gefuetterte. Das Werkzeug meldet, welche Phasen der Pflanzenfolge der
  Plan namentlich deckt, welche nicht, und welche Phasen ohne `target_ec_ms` dastehen. Ohne es
  muesste jeder Konsument diese Pruefung von Hand nachbauen — genau das, was #1244 tun musste.

  Eine Art **ohne** Phasenfolge meldet eine leere Liste, nicht volle Deckung: „nichts zu
  vergleichen" ist nicht „alles in Ordnung".
