...
Status: Teilweise umgesetzt (Framework, API-Key-Auth mit Mehrmandanten-Bindung, Audit, Streamable-HTTP-Transport, Bild-Content und 42 Werkzeuge; Rest des Werkzeugkatalogs und die stdio-Bruecke offen, siehe §4.1 und §9)
Version: 1.5
...
Offen sind insbesondere die Setup-Makros, saemtliche Bulk-Werkzeuge, die IPM- und Ernte-Schreibwerkzeuge, das Aggregat `get_plant_diagnostics` sowie die RAG-Bruecke `search_plant_knowledge`. §2 beschreibt weiterhin den Zielumfang, nicht den Ist-Zustand. `add_plant_diary_entry` ist seit REQ-050 §9 (O-04, entschieden: ja) umgesetzt und faellt aus dieser Liste heraus.
...
- **Werkzeugkatalog vervollstaendigen:** 18 der in §2 spezifizierten Werkzeuge fehlen noch — Setup-Makros, Bulk-Werkzeuge, IPM-/Ernte-/Feeding-Schreibwerkzeuge und die RAG-Bruecke `search_plant_knowledge` (§4.1). `add_plant_diary_entry` ist seit REQ-050 O-04 abgehakt.
