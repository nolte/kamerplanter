...
| AK-EC-04 | **Rückgängig durch die vorherige Adresse (#1848):** Die Info-Mail an die vorherige Adresse enthält einen einmaligen Link, gültig `RETENTION_EMAIL_CHANGE_REVERT_DAYS` (Standard 7 Tage). `POST /privacy/email-change/revert` stellt die vorherige Adresse als bestätigt wieder her, sofern sie noch frei ist (sonst 422), meldet alle Sitzungen ab, entwertet einen offenen Passwort-Reset-Token und widerruft offene E-Mail-Änderungen. Ein unbekannter, bereits verwendeter oder abgelaufener Token antwortet 401. `previous_email` und der Token-Hash werden mit dem Ablauf des Fensters gelöscht (NFR-011 R-07). Nicht widerrufen werden API-Keys (REQ-023 §3.9). | 16 | Unit + Integration |
<!-- /Quelle: #1841 (REQ-023 §3.9) -->
