...
Status: Entwurf
Version: 1.12 (#1788: Kontolöschung löscht den persönlichen Mandanten über das Mandanten-Löschinventar — v1.11 #1768: Löschumfang aus #1761/#1766/#1776 als Abnahmekriterien)
...
|---------|-------|-----------|
| 1.12 | 2026-09-25 | **#1788 Persönlicher Mandant bei Kontolöschung:** Die Kontolöschung behielt den bei der Registrierung angelegten persönlichen Mandanten (REQ-024) samt Standorten, Pflanzen, Tagebuch und Aufgaben; nur Eigentümer, Name und Kurzname wurden ersetzt. Jetzt läuft für jeden persönlichen Mandanten der Person, in dem sie das einzige aktive Mitglied ist, vor dem ArangoDB-Plan das Mandanten-Löschinventar (#1769, `TenantService.delete_tenant`, `origin: account_erasure`): gleiche Liste, eigener Löschnachweis in `tenant_erasure_records`, gleiche Wiederholung. Ernte-, Behandlungs- und Inspektionsdaten bleiben dort unter dem Tombstone-Hash der Person (CanG/PflSchG). **Entscheidung für Mandanten mit weiteren aktiven Mitgliedern:** Die Spezifikation nennt keinen Nachfolger (REQ-049 AK-19 lässt Mitglieder zu); der Mandant bleibt wie bisher erhalten (nur der Kontobezug geht), der Löschauftrag nennt den Grund (`personal_tenants[].outcome = retained_other_members`). Offene Spezifikationsfrage: #1824. Der Löschauftrag ist erst `completed`, wenn der Schritt lief; kann das Deployment keinen Mandanten löschen, hält der Lauf vor jeder Änderung an (503 bzw. ohne verbrauchten Versuch). Neue Abnahmekriterien AK-PT-01 bis AK-PT-03. |
...
2. **Anonymisieren statt löschen**, wenn das Dokument bleiben muss:
   - **gesetzliche Aufbewahrungspflicht** — Ernten und Qualitätsbewertungen (CanG,
     ...
     Freitextnamen werden geleert (Art. 17 Abs. 3).
   - **Datensatz eines möglicherweise geteilten Mandanten** — Aufgaben und
     Aufgabenkommentare, Tagebucheinträge (REQ-050 §7.4), Anhänge (AK-OS-02),
     ...
     Der Kontobezug wird durch den Marker `_anonymized` ersetzt; beim persönlichen
     Mandanten zusätzlich Name und Kurzname.
   - **Ausnahme: der persönliche Mandant, den nur die Person nutzt** (#1788). Ist die
     Person das einzige aktive Mitglied eines ihr gehörenden persönlichen Mandanten,
     ist er kein geteilter Mandant: Vor dem ArangoDB-Plan läuft auf ihm das
     Mandanten-Löschinventar (`TenantErasureEngine.INVENTORY`, #1769) mit
     `origin: account_erasure` — alles gelöscht, nur die Aufbewahrungsdaten (R-16 bis
     R-18) bleiben unter dem Tombstone-Hash. Die Schlüssel dieser Mandanten stehen im
     Löschauftrag (`personal_tenant_keys`), bevor der erste gelöscht wird, das Ergebnis
     je Mandant in `personal_tenants`. Hat der Mandant weitere aktive Mitglieder, bleibt
     er wie oben beschrieben erhalten (Grund im Löschauftrag; wer ihn übernimmt, ist
     offen: #1824).
3. **Pseudonymisieren** für Audit-Protokolle mit eigener Aufbewahrung: Löschaudit
   (NFR-011 R-06, 1 Jahr) und MCP-Aufrufprotokoll (REQ-033). Tombstone-Hash siehe unten.
