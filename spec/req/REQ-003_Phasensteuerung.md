...
Status: Entwurf
Version: 2.15 (Epic #565 Nachzügler: CAM-/Monokarp-/Photoperioden-/Palme-Farn-Geophyt-Feintypisierung + attributgetriebener Sequenz-Resolver)
```
...
| 2.11 | 2026-07-12 | **ADR-006 (Epic #565, E1–E7-Entscheidungs-Gate):** Verweis auf `spec/decisions/ADR-006-perennial-lifecycle-model.md`. Entscheidet den mehrjährigen Outdoor-Lebenszyklus vor der Implementierung: zyklischen Restart-Motor tatsächlich verdrahten (E2, Konsolidierung auf Template-Weg B), Erdbeer-Restart auf `vegetative`/`sprouting` mit `establishment`/`sprouting`-Split (E4), Kopplung Season-State (REQ-047) → Wachstumsphase, instanz-gegatet (E3), Instanz-Override `PlantInstance.cultivation_cycle_type` + `resolve_effective_cycle`-Kaskade (E1/Option C, nach Verdrahtung), `flowering_strategy` bleibt in `species.yaml` + Konsistenz-Check (E5), `cultivation_flexible`-Flag für fakultative Arten (E6), Weg-A-Backfill als Migration v0020 (E7). Reine Entscheidung, keine Code-Änderung in dieser Version. |
