...
| R9 | **The observer also records the abort condition of a deferred decision (#1223).** #1223 deliberately does not build a counter-check against `docker-publish.yml`'s `changes` job and bounds that decision by one condition: *"one observed `changes` job with conclusion `failure` or `cancelled` on a `develop` push"*. Three sweeps re-derived that condition by hand (30, then 60, then 300 runs; `failure`/`cancelled` **0x** each time). The observer therefore computes `decision_triggers` from the JOBS — independently of the verdict, because a cancelled run is `inconclusive` and half of the condition is a cancelled job — and posts **one** comment per `(issue, run, job)` on the issue that records the decision. It records evidence and nothing else: no reopen, no close, no relabel, and no guard is added to `docker-publish.yml`. | #1223 §"What would reopen this — the abort condition, stated so it is checkable" | 0.9 | confirmed (operator text, issue #1223) |

## Out of scope

- The *cause* of the chart-job failure and the outcome check on release artefacts — already delivered by #1224.
- `needs.changes.result` not being consulted by the build jobs — #1223, a different silent path. The observer **detects and records** that issue's abort condition (R9); it does not implement the counter-check #1223 deferred, and deciding whether to build it stays with the operator.
...
| #1223 evidence (R9) | The watched job is named `changes` and appears under that exact name on the jobs endpoint (measured across 300 runs, 2026-08-01 .. 2026-09-20). | Renaming the job in `docker-publish.yml` — the watch would then match nothing and go silent. Nothing detects that today; it is the known limitation of keying on a job name. |
