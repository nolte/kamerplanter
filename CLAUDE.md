...
9. **Multi-tenancy with RBAC Permission Matrix** (REQ-024): Tenant is the isolation container — all resources belong to exactly one tenant. Users can be members of multiple tenants with different roles per tenant (viewer/grower/lead). Assignment-based write control for locations. Platform roles: admin (full KA-Admin) and viewer (read-only admin panel). URL-based routing: `/api/v1/t/{tenant_slug}/...` for tenant-scoped endpoints. Global resources (species, cultivars, IPM data) remain at `/api/v1/...`. Personal tenant auto-created at registration.
    ...
    **What actually enforces access, as of 2026-08-08.** This entry used to name a `require_permission()` FastAPI dependency as the enforcement point; the issue-pattern audit (`.audits/issue-pattern-analysis/2026-08-08-report.md` §3.1) measured **zero** routers using it. What runs in HTTP handlers today is the two-axis pair from `src/backend/app/common/auth.py`: `require_tenant_role(min_role)` for the domain rank and `require_admin_scope(scope)` for administrative surfaces, both resolving the tenant through `get_current_tenant`. Row-level isolation is the `tenant_key=ctx.tenant_key` stamping plus per-repository filtering, not a matrix lookup. The granular resource×action matrix in `src/backend/app/core/permissions.py` exists but is **unwired for HTTP** — its only live consumer is the MCP dispatcher via `assert_mcp_permission` (REQ-033). Wiring it, or retiring it, is open.
