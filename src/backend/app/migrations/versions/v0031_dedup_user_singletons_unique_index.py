...
2. **Ensure the unique index.** A unique persistent index on ``["user_key"]`` is
   created if absent (safe now — step 1 removed the colliding duplicates) and any
   redundant non-unique persistent index on the same field is dropped. Index
   introspection mirrors v0026/v0030: ``indexes()`` filtered by ``type``/
   ``fields``, ``unique`` flag compared, ``delete_index(..., ignore_missing=True)``.

Idempotent (M-3): a re-run finds no duplicate groups (step 1 no-op) and the
unique index already present with no non-unique sibling (step 2 no-op) →
...
class DedupUserSingletonsUniqueIndexMigration(Migration):
    ...
    def up(self, db: StandardDatabase, *, dry_run: bool = False) -> MigrationReport:
        ...
        for collection in _SINGLETON_COLLECTIONS:
            ...
            redundant = [idx for idx in indexes if not idx.get("unique")]
            ...
            # 1. Remove the losing duplicates in a single batched REMOVE.
            if loser_keys:
                db.aql.execute(
            ...
            # 2. Ensure the unique index and drop any non-unique sibling.
            index_changes = 0
            if not has_unique:
                handle.add_persistent_index(fields=_INDEX_FIELDS, unique=True)
                index_changes += 1
