    ...
    for entry in data.get("new_species", []):
        ...
        species_list.append(
            Species(
                ...
                # Whitelist-driven loader: a field not named here is dropped on
                # import without a word. See the same carry in ``seed_plant_info``.
                ...
                pruning_type=entry.get("pruning_type"),
    ...
    for sci_name, fields in data.get("species_enrichment", {}).items():
        ...
        for field, value in fields.items():
            ...
            if field == "watering_guide" and value is not None:
                ...
                # Enrichment lands via a bare ``setattr`` (fill-if-empty), which runs
                # no Pydantic validation — coerce here or raw dicts get persisted.
