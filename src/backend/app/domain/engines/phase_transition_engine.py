...
class PhaseTransitionEngine:
    ...
    def validate_transition(self, plant_key: str, target_phase_key: str) -> list[str]:
        ...
        current_phase = self._phase_repo.get_phase_by_key(plant.current_phase_key or "")
        target_phase = self._phase_repo.get_phase_by_key(target_phase_key)

        if current_phase is None:
            raise PhaseTransitionError("Current phase not found")
    ...
    def execute_transition(
        self, plant_key: str, target_phase_key: str, reason: str = "manual",
    ) -> PlantInstance:
        """Execute a phase transition."""

        self.validate_transition(plant_key, target_phase_key)
        ...
        # Update plant
        plant.current_phase = target_phase.name
