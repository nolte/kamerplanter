...
class ArangoProcessingRestrictionRepository(
    BaseArangoRepository[ProcessingRestriction], IProcessingRestrictionRepository
):
    """ArangoDB persistence for REQ-025 processing restrictions (Art. 18 / 21)."""

    _model_cls = ProcessingRestriction

    def __init__(self, db: StandardDatabase) -> None:
        super().__init__(db, col.PROCESSING_RESTRICTIONS)

    def create(self, restriction: ProcessingRestriction) -> ProcessingRestriction:
        created = super().create(restriction)
