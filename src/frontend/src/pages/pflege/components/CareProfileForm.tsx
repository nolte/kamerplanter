...
export interface CareProfileFormProps {
  /** The profile whose editable fields seed the form state. */
  profile: CareProfile;
  /** Invoked with the saved (or reset) profile so the host refreshes its state. */
  onUpdated: (profile: CareProfile) => void;
  /**
   * Invoked after a successful save or reset. In the dialog wrapper this closes
   * the modal; inline (embedded) hosts typically leave it undefined so the form
   * stays open for further editing.
   */
  onDone?: () => void;
  /**
   * When provided, an explicit "Cancel" action is rendered (dialog usage). Omit
   * it for the always-editable inline placement.
   */
  onCancel?: () => void;
  /**
...
/**
 * The editable body of a plant's care profile. Extracted from
 * ``CareProfileEditDialog`` (#577) so the same form can be rendered inline on the
 * plant-instance Care tab as well as inside the dialog reused by the care
 * dashboard and task-queue list views.
 */
export default function CareProfileForm({
  ...
  }, [profile.plant_key, notification, handleError, onUpdated, onDone, t]);
  ...
  return (
    <Box
      data-testid="care-profile-form"
      sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: embedded ? 0 : 1 }}
    >
      ...
      <Box
        sx={{
          ...
          gap: 1,
          ...(embedded
            ? { borderTop: 1, borderColor: 'divider', pt: 2, mt: 1 }
            : {}),
        }}
      >
        {onCancel && (
