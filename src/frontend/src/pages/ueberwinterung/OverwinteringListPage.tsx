  ...
  const renderMobileCard = useCallback(
    (r: OverwinteringProfile) => (
      <MobileCard
        ...
        subtitleId="winterAction"
        chips={[
          {
            // The chip keeps its per-row `hardiness-chip-<key>` testid, which is
            // an existing contract the keyed hook must not shadow; the id here
            // only ties it to its column for readers that address by column.
            id: 'hardinessRating',
            content: (
              <Tooltip title={hardinessTooltip(r.hardiness_rating)}>
                <Chip
        ...
        fields={[
          { id: 'winterActionMonth', label: t('pages.overwintering.winterActionMonth'), value: monthLabel(r.winter_action_month) },
        ]}
        trailing={
