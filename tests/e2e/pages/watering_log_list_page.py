...
class WateringLogListPage(BasePage):
    ...
    #: `DataTable` renders while the source rows are still there. `PAGE` mounts
    ...
    #: term's debounce is still in flight. The *previous*, unfiltered rows
    #: keep `TABLE_ROWS` satisfied throughout that window, so this anchor is a
    #: no-op there -- a caller reading after `search()` needs
    #: `wait_for_search_applied`/`wait_for_no_search_results` first, which
    #: wait for a *new* thing rather than for "something, anything". This is
    #: exactly what `search()`'s own docstring warns callers about, and what
    #: `test_req004_watering_log.py`'s `test_search_filters_table_rows` was
    #: fixed to do (#946).
    def wait_for_list_content(self, timeout: int = IMPLICIT_WAIT_EQUIVALENT) -> None:
        ...
        not an assertion of its own. A tenant with no watering logs is a state
        the caller's own assertion must still be able to observe.
        """
        with suppress(AssertionError):
            self.wait_for_any_present(
                (self.TABLE_ROWS, self.EMPTY_STATE, self.NO_SEARCH_RESULTS),
                "watering log list content",
                timeout=timeout,
            )
