...
class TankListPage(BasePage):
    ...
    #: wait for a *new* thing rather than for "something, anything". Every
    #: search call site in `test_req014_tank.py` already does that.
    def wait_for_list_content(self, timeout: int = IMPLICIT_WAIT_EQUIVALENT) -> None:
        ...
        not an assertion of its own. A tenant with no tanks is a state the
