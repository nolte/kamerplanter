...
class DiseaseListPage(BasePage):
    ...
    def wait_for_list_content(self, timeout: int = IMPLICIT_WAIT_EQUIVALENT) -> None:
        ...
        not an assertion of its own. A tenant with no diseases catalogued is a
    ...
    def get_row_count(self) -> int:
        ...
        `pytest.skip(...)` or an unconditional presence/absence assertion on
        this, immediately after `open()` -- an unanchored `0` read in the
        pre-fetch window is indistinguishable from a table that genuinely has
        no rows, which is the `has_care_card` defect class this mirrors
        (#946).
        """
