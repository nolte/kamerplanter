...
> **✅ Aufgelöst 2026-07-05 (ehem. E1-Caveat, post-#299) — die 100 %-Werte für REQ-013 und REQ-022 sind jetzt durch echten Code + Tests gedeckt.** Der ursprüngliche Spec-Audit-Caveat vom 2026-07-01 wertete beide als artefakt-präsent, aber semantisch nicht implementiert (`SuccessionPlan` / `OverwinteringProfile` mit 0 Code-Treffern in `src/backend`). Das ist überholt: **`SuccessionPlan` (REQ-013)** wurde in **#361** (`87e5ec63`) und **`OverwinteringProfile` (REQ-022)** in **#360** (`8cd1ee70`) end-to-end implementiert (Model + Repo/Service/API + Edges + Tests). Die Coverage-Werte sind für diese beiden REQs somit korrekt. Siehe `.audits/phase-0-drift-findings.md` (Reconciliation-Update).
>
