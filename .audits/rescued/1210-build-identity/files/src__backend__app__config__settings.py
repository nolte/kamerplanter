...
#: What the health endpoint reports when the running process carries no build
#: stamp — a local checkout, a compose run, the test suite. Deliberately a word
#: no build can produce, so "I don't know which build this is" can never be
#: mistaken for an answer (#1210).
UNKNOWN_BUILD_IDENTITY = "unknown"


class Settings(BaseSettings):
    app_name: str = "Kamerplanter API"
    #: Application version, NOT the build identity. This has read ``1.0.0`` since
    #: the initial commit while the release line ran v0.0.1 → v0.2.0, and
    #: ``.github/release-automation.yml`` records why no file in this repository is
    #: declared version-bearing. It is the OpenAPI ``info.version``, the Sentry
    #: release fallback and the mDNS ``version`` record, so it stays a stable
    #: constant; "which build is running?" is answered by ``build_commit`` /
    #: ``build_timestamp`` below (#1210).
    app_version: str = "1.0.0"
    debug: bool = False

    # Build identity (#1210). Both are stamped into the image *environment* by the
    # container build from the very OCI label values the image already carries
    # (``org.opencontainers.image.revision`` / ``.created``), so the running
    # container and its registry metadata cannot disagree. Empty is the honest
    # default: nothing outside a published image build sets them, and an
    # unstamped process reports ``UNKNOWN_BUILD_IDENTITY`` rather than inventing
    # an identity an operator would act on.
    build_commit: str = ""
    build_timestamp: str = ""
    ...
    def build_identity(self) -> dict[str, str]:
        """Return the identity of the build this process runs, for ``GET /api/health``.

        Answers the question an operator has after a merge: *does the instance in
        front of me already contain that commit?* Before #1210 the only build-ish
        field on the endpoint was ``version``, a constant that matched no published
        release, so the question could only be answered by provoking behaviour.

        Both values are unstamped outside a published image build, and an unstamped
        process says so: :data:`UNKNOWN_BUILD_IDENTITY` rather than a plausible-
        looking placeholder. A wrong build identity is worse than a missing one —
        it is the one field an operator would act on without re-checking.

        Returns:
            ``commit`` (git SHA of the build) and ``built_at`` (RFC-3339 build
            timestamp), each either the stamped value or ``"unknown"``. Nothing
            else: the endpoint is unauthenticated, and the SHA of a public
            repository is the only thing here that is already public anyway.
        """
        return {
            "commit": self.build_commit.strip() or UNKNOWN_BUILD_IDENTITY,
            "built_at": self.build_timestamp.strip() or UNKNOWN_BUILD_IDENTITY,
        }
