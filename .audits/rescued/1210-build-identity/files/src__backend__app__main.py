...
def root_health() -> dict:
    ...
    Carries ``commit``/``built_at`` (#1210) for the same reason: an operator asking
    "does the running instance already contain the fix that merged two days ago?"
    must be able to ask before authenticating, and had no field to ask it with —
    ``version`` is a constant that matches no published release, see
    :meth:`Settings.build_identity`. An unstamped build reports ``"unknown"``
    rather than guessing.

    ``status``, ``version``, ``mode`` and ``supported_majors`` keep their names and
    meanings: the Home Assistant integration and the major negotiation read them.
    """
