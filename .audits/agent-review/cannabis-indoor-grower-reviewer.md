...
- [ ] [agent-management.output-shape] System prompt names a report path (`spec/analysis/cannabis-indoor-grower-review.md`) and a markdown template, but the structured output shape the parent consumes is not stated upfront; agent dispatch is fire-and-forget per `skill-vs-agent`.
