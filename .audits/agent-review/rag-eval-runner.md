...
- [ ] [agent-review.Read-only-tools-rule] Description names only read-only verbs ("Fuehrt ... aus, interpretiert, klassifiziert, schlaegt ... vor"), but `tools` declares `Edit`, `Write`, and `Bash` — per `agent-review.Checks-derived-from-agent-management` ("read-only agents MUST NOT receive write, edit, or execution tools"), this is a BLOCKER.
