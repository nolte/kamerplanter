...
- **P1–P4** — dispatched as one sequential chain to `mkdocs-documentation`. All four implemented, none refuted; commits `d664fd4ff`, `3ebc573f0`, `db3bf54c5`, `7c7b0a865`. The specialist reported two scope observations worth keeping: the existing 2.6-day threshold measurement was already correct and was left untouched, and the existing `0.0.23` mentions were already dated to 2026-08-17, so P2 needed no correction there — only the missing failure-mode statement.
- **Verification finding (orchestrator, not the specialist)** — P3's documented `curl` was **not runnable as written**. It sent only `Accept: application/vnd.oci.image.manifest.v1+json`; these tags are multi-arch, the OCI revision annotation sits on the *index*, and GHCR answered that request with a body whose annotation is `null`. A documented check that silently returns nothing is the same failure class this issue exists to remove, so it was fixed fix-forward in `a525529f6` (index media type added, plus a comment saying why it must stay) and re-run end to end: the corrected command returns `b40b3ccd8393a612876bdf8c48ad0144f81e32c3`, matching the value the surrounding prose quotes.
- **Gate** — `task docs:build` exits 0 with no ERROR; the two anchors P4 introduces (`ci-cd.md#haeufige-fragen`, `ci-cd.md#frequently-asked-questions`) resolve. The required `static` lane's pre-commit suite passed on every commit.
