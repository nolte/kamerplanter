...
- **P10** → `general-purpose` (spec skill not runtime-resolvable) → **done**, commit `78363814f`, +62 lines to REQ-023 (§3.8 service/API, §4.6 frontend, changelog v1.12, Nicht-in-Scope audit-log line, endpoint count ~23→~25). CSRF double-submit table qualified precisely — split `POST /auth/refresh` into cookie-path (Double-Submit) and body-path (#1118, "kein ambient Credential") rows in BOTH §1 table (~L163) and §3.3 table (~L994); no blanket removal, logout/logout-all untouched. Rate limit documented honestly as **global** (slowapi) with per-IP lockout separate — matches F-1. Every claim traced to source. Structural note: §3 numbering already non-monotonic (3.7 M2M before 3.3), so new section is §3.8 at end of §3. `task check` exit 201 = only the two known pre-existing failures. NOTE: this commit swept in the untracked `analysis.md` at its P9 state (broad `git add`) — harmless, `git rm`'d before merge.
- **P11** → `nolte-engineering:e2e-test-generator` → **done**, commit `9b7a87f02` (4 files, +514). TC-023-073..076 (dialog open / QR non-empty / countdown ticks / refresh → different QR), `test_count` 72→76, traceability green, `--collect-only` + `-m req023` clean. Empty-QR fails by construction (`qr_path_ds` waits ≥2 `<path>`, times out on empty); "QR changed" compares only the data path (background square regex-filtered). No fixed sleeps. **Live E2E pending CI** (full Docker Selenium stack, full mode) — not faked; handoff: `task test:e2e -- -m req023 -k device_pairing`. Feature test → no pre-fix negative verification required. No missing frontend hooks. Deviation: refresh driven via close+reopen (same `requestCode()` path) rather than a 60–120s expiry wait — documented.

## Verification phase

- All 11 packages done. Commit chain `f6d8c893a..9b7a87f02` (11 commits) on `feat/1118-qr-device-pairing`.
- Quality-gate baseline (P10 `task check`): exit 201, only two pre-existing failures unrelated to this strand — boto3 missing on `test_s3_adapter.py` (local env gap) and `AccountSettingsPageFlows` HA flake under parallel load. All lint/typecheck/7-custom-gates/side-services green; backend 6962 passed/3 failed, frontend 3820 passed/1 failed.
- Security-sensitive paths for the review chain: `auth_service.py` (unauthenticated redemption minting a full token pair; lockout order; log hygiene), `api/v1/auth/router.py` (public redeem, refresh body path CSRF omission, D1 client-IP tradeoff), `schemas.py`, `data_access/external/redis_device_pairing.py` + `device_pairing_throttle.py`, `common/dependencies.py`, plus the persisted client-supplied `device_name` rendered in the session list (P5).
- Follow-up-issue candidates for PR time: (F-1) global vs per-IP redemption rate limit + the wrong `settings.py` comment; `/auth/refresh` carries no rate limit; Nuclei brute-force template; `pages.auth.*` Sie/du sweep; `AccountSettingsPageFlows` HA flake under coverage load.

## Scope addition — P12 (operator, 2026-08-12)

After the strand was verify-green, the operator requested a QR code that is **also shown in light mode**, carrying only minimal info like the URL (no login, no credential). This is a deliberate scope addition, folded into this PR (operator decision). It does NOT touch the pairing endpoints, which stay 404 in light mode.

**Operator decisions:** frontend-only (no backend, no code — the instance already knows its own URL); folded into PR #1118; the existing dialog is made mode-aware.

**Repo facts grounding it:** `/settings` (AccountSettingsPage) is reachable in light mode, but the light-mode tab set (`modules, dashboard, apikeys, ha, …`) has **no** `sessions` tab — the current pairing entry (in the sessions tab) is full-mode-only. `runtimeConfig` exposes NO base URL, so the light-mode QR derives its URL from `window.location.origin` (the URL the user actually reaches the instance through). Full-mode payload is `{v,url,code}`; the light-mode payload is `{v,url}` (code absent) — a scanning app distinguishes login-pairing from instance-discovery by the presence of `code`, forward-compatible under the same `v`.

### P12 — Light-mode instance-discovery QR (frontend-only, URL-only)

- **Problem statement**: a light-mode (anonymous) user has no way to point a mobile app at this instance; the full pairing flow is correctly unavailable there.
- **Acceptance criteria**:
  - `ConnectDeviceDialog` becomes mode-aware. Full mode: unchanged (fetch one-time code, `{v,url,code}`, countdown, expiry/refresh). Light mode: renders a **URL-only** QR `{"v":1,"url":<window.location.origin>}` — **no** backend call, no code, no countdown, no expiry/refresh — with light-mode copy (points the app at this instance; explicitly NOT "logs you in / grants account access").
  - vitest: in light mode the dialog makes **no** `createDevicePairing` API call (assert the endpoint fn is not invoked), the QR `value` contains the origin URL and no `code` field, and there is no countdown/expired affordance.
  - Entry point reachable in light mode: a light-mode-visible location in `AccountSettingsPage` (a `connect`/"App verbinden" surface in the light-mode tab set). Full mode keeps its pairing entry in the sessions tab — no duplication. vitest: the entry is present in light mode and the light-mode dialog is the URL-only variant.
  - Pairing endpoints still 404 in light mode (unchanged — assert nothing regressed there).
  - i18n keys for the light-mode variant copy in **both** de and en; no hard-coded strings.
  - Docs/spec kept truthful: a short note in `docs/{de,en}/api/authentication.md` and REQ-023 §4.6 that a light-mode URL-only variant exists (code absent), so the contract doesn't drift.
- **Touched files**: `src/frontend/src/pages/auth/ConnectDeviceDialog.tsx`, `AccountSettingsPage.tsx`, `src/frontend/src/i18n/locales/{de,en}/pages.json`, `src/frontend/src/test/pages/ConnectDeviceDialog.test.tsx` (+ maybe a small AccountSettings test), `docs/{de,en}/api/authentication.md`, `spec/req/REQ-023_Benutzerverwaltung-Authentifizierung.md`.
- **Specialist**: `nolte-engineering:fullstack-developer` (verify: `nolte-engineering:frontend-code-reviewer`).
- **Depends on**: P7 (done).
