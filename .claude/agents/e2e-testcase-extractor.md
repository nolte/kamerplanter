...
description: Extrahiert End-to-End-Testfälle aus Anforderungsdokumenten (spec/req/, spec/nfr/), gleicht bestehende Testabdeckung gegen Spezifikationen ab, erstellt/aktualisiert RAG-optimierte Testfall-Dokumente und stellt Traceability zwischen Anforderungen und Testszenarien her. Aktiviere diesen Agenten wenn aus einem oder mehreren REQ-/NFR-Dokumenten systematisch alle Testszenarien abgeleitet oder eine Coverage-Gap-Analyse gegen bestehende Tests durchgeführt werden soll. Nicht für Selenium-E2E-Code-Generierung — dafür `selenium-test-generator`; nicht für Unit-Test-Läufe — dafür `unit-test-runner`.
