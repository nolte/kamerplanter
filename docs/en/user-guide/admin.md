...
!!! danger "Deleting a tenant is irreversible"
    ...
    A confirmation dialog additionally requires you to re-confirm before the deletion runs: enter the tenant's slug and, if your account has a local password, your current password. If you sign in only through a federated provider (Google, GitHub, Apple, OIDC), the password field is omitted and the slug alone confirms the deletion. If the entered slug does not match, the action reports `422`; if the password is missing or wrong, it reports `401` — in both cases the tenant stays unchanged. A service account cannot perform this action, and neither can signing in with a personal API key instead of a signed-in session. After five wrong password attempts the system locks the confirmation for 15 minutes — repeated failures double the wait time up to 4 hours (`429`, the message states the wait time); the same budget applies account-wide to confirming an account deletion and a password change too. <!-- Issue #1791, #1813, #1814, #1816 -->
...
!!! danger "Deleting a user account is immediate and complete"
    A confirmation dialog requires you to re-confirm before the deletion runs: enter the **target account's email address** and your **own** current password, if your admin account has a local password — not the password of the account being deleted. If the entered email address does not match, the action reports `422`; if your password is missing or wrong, it reports `401`. You cannot delete your own account this way (`403`). After five wrong password attempts the system locks the confirmation for 15 minutes — repeated failures double the wait time up to 4 hours (`429`, the message states the wait time); the same budget applies account-wide to tenant deletion and password changes too. <!-- Issue #1813, #1814, #1816 -->

    Once confirmed successfully, the user account is deactivated at once, all sessions end, and the full account erasure runs immediately (see [Data Retention & Anonymization](../guides/data-retention.md)). An erasure request is persisted as proof — the same as when a user deletes their own account; it additionally carries a salted reference to you as the confirming admin, never your account key in the clear.
