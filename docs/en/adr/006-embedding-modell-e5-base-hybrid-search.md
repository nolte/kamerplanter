...
## Amendment (2026-09-24): model downloads pinned and verified

As of 2026-09-24, all four model downloads of the `embedding-service` (`multilingual-e5-small`, `-base`, `-large`, and the MiniLM model, which remains available as a build target) are pinned to a fixed commit revision at Docker build time; every kept file is additionally verified in the same build stage via `sha256sum -c --strict` (tracked internally as issue #1724). The MiniLM model is a third-party export (`Xenova/paraphrase-multilingual-MiniLM-L12-v2`), not the model authors' own repository — pinning does not change that; it only makes reproducible which version of it ends up in the image.

**Measured** against the previously unpinned image: every downloaded file byte-identical (sha256), and the embeddings generated by all four models identical (maximum delta 0.0).

**Found while measuring:** the `--target e5-small` build target used to answer every `/embed` request with an error, because its ONNX graph declares an input field (`token_type_ids`) its tokenizer never produces. The feed is now built from the fields the graph itself declares, no longer from whatever the tokenizer happens to produce.

The service also now runs with `HF_HUB_OFFLINE=1` at runtime: a `from_pretrained` call with a hub ID, or an accidental `snapshot_download`, now fails instead of silently fetching an unpinned model.

This ADR's actual decision — switching to `multilingual-e5-base`/`-large` and Hybrid Search — remains unchanged; only how reproducibly and verifiably the model files reach the image has changed.
