...
!!! info "CI: smoke gate per PR + nightly full run"
    The suite also runs in GitHub Actions — using the same Docker Compose stack as local runs: the `e2e-smoke` workflow runs the fast smoke profile on every pull request and on pushes to `develop`. It was a required check on `develop` from ADR-011 until 2026-07-29 and is **advisory** since: it still runs and still reports on every pull request, but it no longer blocks the merge — read it before you merge. The rollback was driven by merge latency of roughly 11 minutes per run; the nightly full-matrix run now carries the coverage. Whether the suite actually runs is still decided by a job inside the workflow rather than by a path filter on the trigger: a required workflow skipped by path filtering never reports a result and blocks the pull request indefinitely — that construction stays in place so the check can be re-armed at any time. The `e2e-nightly` workflow runs the complete suite nightly as a matrix over the compose profiles `light`, `full`, `mobile`, `tablet`, and `full-mobile`; a failing run no longer opens a GitHub issue — the run status, the per-profile rendered check run, and the artifacts carry everything the automatic issue merely restated. Test protocol, screenshots, and container logs are attached to every run as workflow artifacts.
