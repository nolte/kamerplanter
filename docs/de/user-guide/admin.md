...
!!! danger "Mandanten löschen ist irreversibel"
    ...
    Ein Bestätigungsdialog verlangt zusätzlich eine erneute Bestätigung, bevor die Löschung ausgeführt wird: Trage den Kurznamen (Slug) des Mandanten ein und, sofern dein Konto ein lokales Passwort hat, dein aktuelles Passwort. Meldest du dich ausschließlich über einen externen Anbieter (Google, GitHub, Apple, OIDC) an, entfällt das Passwortfeld und der Kurzname allein bestätigt die Löschung. Stimmt der eingegebene Kurzname nicht, meldet die Aktion `422`; fehlt das Passwort oder ist es falsch, meldet sie `401` — in beiden Fällen bleibt der Mandant unverändert. Ein Dienstkonto kann diese Aktion nicht ausführen, ebenso wenig eine Anmeldung mit einem persönlichen API-Schlüssel statt einer angemeldeten Sitzung. Nach fünf falschen Passworteingaben sperrt das System die Bestätigung für 15 Minuten — bei wiederholten Fehlversuchen verdoppelt sich die Wartezeit bis zu 4 Stunden (`429`, Meldung nennt die Wartezeit); dasselbe Budget gilt kontoweit auch für die Bestätigung einer Konto-Löschung und einer Passwortänderung. <!-- Issue #1791, #1813, #1814, #1816 -->
...
!!! danger "Nutzerkonto löschen ist sofort und vollständig"
    Ein Bestätigungsdialog verlangt vor der Löschung eine erneute Bestätigung: Trage die **E-Mail-Adresse des Zielkontos** ein und gib **dein eigenes** aktuelles Passwort ein, sofern dein Admin-Konto ein lokales Passwort hat — nicht das Passwort des zu löschenden Kontos. Stimmt die eingegebene E-Mail-Adresse nicht, meldet die Aktion `422`; fehlt dein Passwort oder ist es falsch, meldet sie `401`. Dein eigenes Konto kannst du auf diesem Weg nicht löschen (`403`). Nach fünf falschen Passworteingaben sperrt das System die Bestätigung für 15 Minuten — bei wiederholten Fehlversuchen verdoppelt sich die Wartezeit bis zu 4 Stunden (`429`, Meldung nennt die Wartezeit); dasselbe Budget gilt kontoweit auch für Mandantenlöschung und Passwortänderung. <!-- Issue #1813, #1814, #1816 -->

    Bestätigst du erfolgreich, wird das Nutzerkonto sofort deaktiviert, alle Sitzungen werden beendet, und die vollständige Konto-Löschung läuft unmittelbar (siehe [Datenaufbewahrung & Anonymisierung](../guides/data-retention.md)). Ein Löschantrag wird dabei als Nachweis gespeichert — genau wie bei einer Löschung, die ein Nutzer selbst über sein Konto anstößt; er hält zusätzlich eine gesalzene Referenz auf dich als bestätigenden Admin, nie deinen Kontoschlüssel im Klartext.
