...
| `STORAGE_TASK_PHOTO_ORPHAN_HOURS` | `48` | Nein | Wie lange ein Aufgabenfoto unreferenziert liegen darf, bevor der nächtliche Aufräum-Task es **löscht** (#1393). Jeder Upload ist kurzzeitig unreferenziert — der Anhang wird geschrieben, bevor das Formular abgeschickt wird, das ihn verknüpft —, deshalb ist der Wert ein weiter Sicherheitsabstand und kein Feineinstellungs-Regler: Absenken Richtung 0 löscht Fotos, an deren Formular gerade noch jemand sitzt. `0` schaltet den Aufräum-Task ab. |
