...
- `months` (`list[int]`, Wertebereich 1–12) — Monate, in denen diese Methode am besten geeignet ist; serverseitig dedupliziert und aufsteigend sortiert. Erfasst nur vegetative Vermehrungszeitpunkte; Aussaatzeiträume für die Samenvermehrung werden über die separaten Felder `direct_sow_months`, `indoor_start_months` und `transplant_months` abgebildet.
