...
Integrationstests unter `tests/integration/` brauchen eine laufende ArangoDB-Instanz — sie sind die einzige Prüfung der Repository- und AQL-Schicht gegen eine echte Datenbank. In CI laufen sie als Pflicht-Lane (`Integration tests (ArangoDB)` in `backend-guards.yml`) gegen einen Service-Container.
...
class TestEcBudgetCalculator:
    ...
    def test_ec_net_calculation(self):
        # Netto-EC-Budget = Ziel-EC minus Basiswasser-EC
    ...
    def test_temperature_correction_to_25c(self):
        # EC-Messwerte werden auf 25 °C Referenztemperatur korrigiert
...
Das legt `tests/e2e/.venv` aus dem hash-geprüften `tests/e2e/uv.lock` an — genau
die Installation, die auch das E2E-Image ausführt (`tests/e2e/Dockerfile`). Nimm
es in den PATH auf (`export PATH="$PWD/tests/e2e/.venv/bin:$PATH"`) oder stelle
den folgenden Kommandos `tests/e2e/.venv/bin/python -m` voran.

Abhängigkeiten: `selenium>=4.25.0,<5`, `webdriver-manager>=4.0.0`,
`pytest>=8.3.0` — deklariert in `tests/e2e/pyproject.toml`.
